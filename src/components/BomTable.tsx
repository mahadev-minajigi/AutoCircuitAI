    ...
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)' }}>Complete list of parts required for the build.</p>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid var(--border-color)' }}>
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-muted)' }}>Ref</th>
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-muted)' }}>Component Name</th>
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-muted)' }}>Category</th>
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-muted)' }}>Package/Footprint</th>
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-muted)', textAlign: 'right' }}>Est. Cost</th>
            </tr>
          </thead>
          <tbody>
            {design.components.map((comp) => (
              <tr key={comp.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <td style={{ padding: '16px', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>{comp.id.toUpperCase()}</td>
                <td style={{ padding: '16px' }}>
                  <div style={{ fontWeight: 600 }}>{comp.name}</div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{comp.description}</div>
                </td>
                <td style={{ padding: '16px' }}>
                  <span className="badge badge-cyan">{comp.category}</span>
                </td>
                <td style={{ padding: '16px', fontFamily: 'var(--font-mono)' }}>{comp.package}</td>
