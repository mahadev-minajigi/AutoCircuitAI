...
const PinoutTable: React.FC<PinoutTableProps> = ({ design }) => {
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <h2 style={{ fontSize: '1.2rem', fontWeight: 600 }}>Pin Connection Matrix</h2>
      <p style={{ color: 'var(--text-muted)' }}>Map of MCU pins to component pins and protocols.</p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid var(--border-color)' }}>
              ...
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-muted)' }}>Protocol</th>
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-muted)' }}>Target Component</th>
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-muted)' }}>Target Pin</th>
              <th style={{ padding: '16px', fontWeight: 600, color: 'var(--text-muted)' }}>Description</th>
            </tr>
          </thead>
          <tbody>
              ...
              return (
                <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <td style={{ padding: '16px' }}>
                    <span className={`badge ${pin.protocol === 'Power' ? 'badge-emerald' : pin.protocol === 'I2C' ? 'badge-amber' : 'badge-cyan'}`}>
                      {pin.protocol}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: '16px', color: 'var(--text-muted)', fontSize: '0.9rem' }}>{pin.description}</td>
                </tr>
              );
            })}
          </tbody>
